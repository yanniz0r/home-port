...
const UNSPLASH_SECRET_ACCESS_KEY =
  process.env.VUE_APP_UNSPLASH_SECRET_ACCESS_KEY;

const unsplash =
  UNSPLASH_ACCESS_KEY &&
  UNSPLASH_SECRET_ACCESS_KEY &&
  new Unsplash({
...
const store = createStore<State>({
  ...
  actions: {
    ...
    async setRandomUnsplashBackgroundImage(store) {
      ...
      if (!topic) {
        console.warn(
          "🚨 No topic for unsplash set! Please tell us what you want to see."
        );
        return;
      }
      if (!unsplash) {
        console.warn(
          "🚨 No credentials for unsplash set! Make sure you provide them."
        );
        return;
      }
      ...
      if (!images.length) {
        console.warn(
          `🚨 "${topic}" seems to be something noone makes photos of... Maybe try something else. I heard "cats" always works.`
        );
      }
      const selectedImageUrl =
        images[Math.floor(Math.random() * images.length)].urls[
          store.state.settings.backgroundImageUnsplashSize || "regular"
        ];
      console.info(
        "Got a beautiful new background image from unsplash:",
        selectedImageUrl
      );
      store.commit("setSettings", {
        ...store.state.settings,
